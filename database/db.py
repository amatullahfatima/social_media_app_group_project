...
def deactivate_user_db(conn, email):
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM users WHERE email = ?", (email,))
        user = cursor.fetchone()
        ...
        if user["is_active"] == 0:
            messagebox.showinfo(
                "Info", f"User '{email}' is already deactivated")
            return
        cursor.execute(
        ...
        messagebox.showerror(
            "Error", f"Database error while deactivating user: str{e}")
        return False
    finally:
        conn.close()


def reactivate_user_db(conn, email):
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM users WHERE email = ?", (email,))
        user = cursor.fetchone()
        ...
        messagebox.showerror(
            "Error", "Database error while reactivating user: str{e}")
